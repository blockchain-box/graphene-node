...
__export(index_exports, {
  UdpConnPiper: () => UdpConnPiper,
  UdpSocket: () => UdpSocket,
  connPiper: () => connPiper,
  createTcpProxy: () => createTcpProxy,
  createUdpFramedProxy: () => createUdpFramedProxy,
  pipeTcpServer: () => pipeTcpServer,
  pipeUdpFramedServer: () => pipeUdpFramedServer,
...
var UdpSocket = class {
  ...
  constructor(opts) {
    ...
    this.server = import_dgram.default.createSocket("udp4");
    this.client = import_dgram.default.createSocket("udp4");
...
function createTcpProxy(listenOpts, connectRemote, piperOpts, stats, onListen) {
  const proxy = import_net.default.createServer({ allowHalfOpen: true }, (c) => {
...
function createUdpFramedProxy(listenOpts, connectRemote, logger, onBind) {
  const proxySocket = import_dgram.default.createSocket("udp4");
...
function pipeUdpFramedServer(remoteStream, localOpts, logger, _stats) {
  const localSocket = import_dgram.default.createSocket("udp4");
